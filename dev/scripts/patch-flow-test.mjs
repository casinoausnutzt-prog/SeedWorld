...
async function ensureGatePolicy(repo) {
  await mkdir(join(repo, 'app', 'src', 'llm'), { recursive: true });
  await writeJson(join(repo, 'app', 'src', 'llm', 'llm-gate-policy.json'), {
...
async function testLocking() {
  ...
  const auditRaw = await readFile(join(repo, 'runtime/.patch-manager', 'audit.jsonl'), 'utf8');
