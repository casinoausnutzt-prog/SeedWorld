...
const STATE_DIR = path.join(ROOT, "runtime", ".patch-manager");
...
const MANIFEST_SCHEMA_FILE = path.join(ROOT, "app", "src", "sot", "patches.schema.json");
...
const PROTECTED_GATE_FILES = new Set([
  "app/src/kernel/store/applyPatches.js",
  "app/src/kernel/store/createStore.js",
  "app/src/kernel/llmGovernance.js",
...
async function runPatchQueue(options = {}) {
  ...
  if (gates.runPreflightBefore) {
    await runGateCommand("before", "node dev/tools/runtime/preflight.mjs", gates.softFailPreflight);
  }
  ...
  if (state.status.mode === "completed") {
    ...
    if (gates.runPreflightAfter) {
      await runGateCommand("after", "node dev/tools/runtime/preflight.mjs", gates.softFailPreflight);
    }
  }
